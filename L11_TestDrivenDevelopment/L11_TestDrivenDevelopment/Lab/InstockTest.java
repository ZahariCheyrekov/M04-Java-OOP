...
public class InstockTest {
    ...
    @Test
    public void TestShouldCheckIfCollectionContainsProduct() {
        Product product = new Product("productName1", 10.00, 10);
    ...
    @Test
    public void testShouldReturnTheCountOfTheCollection() {
        createAndAddProducts(this.stock);
        assertEquals(4, this.stock.getCount());
    }

    @Test
    public void testShouldReturnSizeOfTheCollection() {
        Product[] addedProducts = createAndAddProducts(this.stock);

        int sizeOfCollection = addedProducts.length;
        assertEquals(4, sizeOfCollection);
    }
    ...
    @Test
    public void testFindElementAtSpecifiedIndexFromCollection() {
        createAndAddProducts(this.stock);
        stock.find(2);
    }

    @Test
    public void testShouldChangeAndUpdateTheProductQuantity() {
        Product[] addedProducts = createAndAddProducts(this.stock);
    ...
    public void testChangeQuantityShouldFailWhenNoSuchProductInStock() {
        createAndAddProducts(this.stock);
        Product notAddedProduct = new Product("noSuchProduct", 9.99, 1);
        this.stock.changeQuantity(notAddedProduct.getLabel(), notAddedProduct.getQuantity() + 13);
    }

    @Test
    public void testShouldFindTheProductByTheSpecifiedLabel() {
        Product[] addedProducts = createAndAddProducts(this.stock);
        String expectedProduct = addedProducts[1].getLabel();

        Product product = this.stock.findByLabel(expectedProduct);
        assertNotNull(product);

        String actualProduct = product.getLabel();
        assertEquals(expectedProduct, actualProduct);
    }

    @Test(expected = IllegalArgumentException.class)
    public void testShouldThrowExceptionForNoSuchItemWithTheSpecifiedLabel() {
        String productToSearch = "lemons";
        this.stock.findByLabel(productToSearch);
    }

    @Test
    public void testShouldFindFirstByAlphabeticalOrderElement() {
        Product[] addedProducts = createAndAddProducts(this.stock);
    ...
    private static Product[] createProducts() {
        return new Product[]{
                new Product("testProduct4", 10.00, 13),
                new Product("testProduct2", 9.99, 42),
                new Product("testProduct3", 50.00, 13),
                new Product("testProduct1", 50.99, 79)
        };
    }
